...
const wrapper = shallowMount(AllImagesView, { router, localVue })
...
jest.mock('@/utils/communication', () => {
    return {
        request_json: (url, method,) => {
            ...
            else if (url == "/pictures/pictures_search" && method == "get")
                return Promise.resolve({
                    code: 200,
                    data: [{
                        id: 1,
...
describe('search pictures', () => {

    //HERE IS HOW TO TEST MODULE BEHAVIORS WITH THE MOCKED API
    it(('test mounted list'), async () => {
        expect(wrapper.vm.images_list[0]).toEqual({
            id: 1,
            title: "test",
            intro: "test",
            likesNum: 10,
    ...
    it(('test mounted images num'), async () => {
        expect(wrapper.vm.images_list.length).toBe(1)
    })
