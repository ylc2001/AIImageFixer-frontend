<template>
  <div class="main-body">
    ...
    <div class="like-collect">
      ...
      <i
        v-if="exhibition.collection == false"
      ...
      <el-button class="threed-button" @click="go3D()"> 3D游览 </el-button>
    </div>
    ...
    <div>
      <div class="search-title">
        <el-input v-model="search_info.title" placeholder="请输入搜索标题">
          <el-button
            class="searchbutton"
            slot="append"
            icon="el-icon-search"
            @click="
              Search(search_info.order_type, search_info.order, 0, page_size)
            "
            >搜索</el-button
          >
        </el-input>
      ...
      <div class="other-search">
        ...
        <div class="sort">
          <span style="color: #777777; margin-right: 1%">排序方式</span>
      ...
      <div class="main-content">
        <section
          v-for="(picture, index) in pics"
          v-bind:key="picture.index"
          class="my-section"
        >
          <div class="container">
            <div class="imgBx jarallax" @click="handleImgPreview(index)">
              <img class="jarallax-img" :src="picture.pic" />
            </div>
            <div class="content" data-jarallax-element="-200 0">
              ...
              <div class="like-collection-comment">
                <i
                  v-if="picture.like == false"
                  class="iconfont icon-dianzan"
                  style="font-size: 20px; cursor: pointer"
                  @click="likePic(picture.id)"
                >
                </i>
                <i
                  v-else
                  class="iconfont icon-dianzan1"
                  style="font-size: 20px; cursor: pointer"
                  @click="cancelLikePic(picture.id)"
                >
                </i>
                ...
                <i
                  ...
                  class="el-icon-star-on"
                  style="font-size: 20px; cursor: pointer"
                  @click="cancelCollectPic(picture.id)"
                >
                </i>
                <span style="font-size: 15px">
              ...
              <div class="repair-detail">
                <span class="repair-detail-text" @click="toImage(picture.id)">
                  查看图片详情->
                </span>
              </div>
            </div>
    ...
    <div class="comments">
      <comment_beta
        ...
        style="margin-left: 10%; margin-right: 10%; margin-top: 50px"
      >
      </comment_beta>
    </div>
        ...
        <el-main>
          <div class="drawer-comment">
            <comment_beta
              ...
              v-bind:timestamp="comment.time"
              v-bind:deletable="comment.deletable"
              v-bind:delete_comment="delete_comment_pic"
...
export default {
  ...
  methods: {
    go3D() {
      ...
      CookieOperation.setCookie("back", 0, 10e9);
          ...
          if (tmp.code == 400) {
            ...
            for (let i = 0; i < tmp.data.pictures.length; i++) {
              ...
              this.pics.push({
                ...
                collection: tmp.data.pictures[i].collection,
                collectionNum: tmp.data.pictures[i].collectionNum,
          ...
          if (response.code == 200) {
            ...
            for (let i in this.pics) {
              if (this.pics[i].id == id) {
                this.pics[i].like = true;
                this.pics[i].likesNum++;
              }
            }
          } else {
      ...
      request_json(API.collection.path, API.collection.method, {
        ...
        .then((response) => {
          if (response.code == 200) {
            ...
            for (let i in this.pics) {
              if (this.pics[i].id == id) {
                this.pics[i].collection = true;
                this.pics[i].collectionNum++;
              }
            }
          } else {
            this.$message(response.data);
        ...
        .catch((error) => {
          console.log("error in colle: ", error);
        });
    },
    cancelCollectPic(id) {
      request_json(API.collection.path, API.collection.method, {
        ...
        .catch((error) => {
          console.log("error in cancelcalle: ", error);
        });
    },
      ...
      request_json(API.picture_search.path, API.picture_search.method, {
        ...
        if (response.code == 400) {
          ...
          for (let i = 0; i < response.data.comments.length; i++) {
            if (response.data.comments[i].is_delete == 1) {
              this.dialog_pic.comments[i].deletable = true;
            } else {
              this.dialog_pic.comments[i].deletable = false;
            }
          }
        }
      });
          ...
          request_json(API.delete_comments.path, API.delete_comments.method, {
            ...
            .then((response) => {
              if (response.code == 200) {
                this.$message("删除成功");
                this.getExhibitionData(this.exhibition.whichExbt.id);
                this.openDialog(this.dialog_pic.id);
              } else {
                this.$message(response.data);
              }
            })
...
.main-body:before {
  ...
  font-weight: 700;
  color: transparent;
  -webkit-text-stroke: 13px rgb(242, 242, 242);
  z-index: -1;
}
...
.exhibit_images:hover {
  box-shadow: 0 2px 30px 0 #5b5b5c;
  min-width: 70%;
  max-width: 100%;
  min-height: 100%;
  object-fit: contain;
...
section .container .imgBx:hover {
  box-shadow: 0 0 30px 10px rgb(104, 104, 104);
  cursor: pointer;
}
...
section .container .content {
  ...
  max-width: 600px;
  color: #fff;
  box-shadow: 0 0 10px rgba(237, 237, 239, 0.5);
  border-radius: 15px;
}
